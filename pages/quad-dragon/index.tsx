...
export async function getStaticProps() {
  const data = await fetchNs('quadDragon')
  return {
    props: {
      robes: data.robes,
...
const IndexPage = ({ robes, lastUpdate }: Props) => {
  return (
    <div className="py-3 md:pb-0 font-mono flex flex-col justify-center items-center gap-4 pt-10 md:w-screen">
      <h1 className="text-lg md:text-3xl">Four Dragons</h1>
      <div className="text-center max-w-screen-md md:leading-loose">
        <p className="md:text-xl">
          There are {robes.length} Adventure Card Starter Decks for sale containing Four Dragons. The floor price is {robes[0].price} ETH.
        </p>
