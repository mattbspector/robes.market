...
export async function getStaticProps() {
  const data = await fetchNs('invisiblityCloak')
  return {
    props: {
      robes: data.robes,
...
const IndexPage = ({ robes, lastUpdate }: Props) => {
  return (
    <div className="py-3 md:pb-0 font-mono flex flex-col justify-center items-center gap-4 pt-10 md:w-screen">
      <h1 className="text-xs md:text-3xl">⚡️ 🪄 Harry Potter and his Invisibility Cloak 🪄 ⚡️</h1>
      <div className="text-center max-w-screen-md md:leading-loose">
        <p className="text-xs md:text-xl">
          There are {robes.length} Adventure Card Starter Decks for sale containing a Wizard AND Invisibility AND a Cloak. The floor price is {robes[0].price} ETH.
        </p>
